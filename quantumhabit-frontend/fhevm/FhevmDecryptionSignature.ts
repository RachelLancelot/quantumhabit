...
export class FhevmDecryptionSignature {
  ...
  static checkIs(s: unknown): s is FhevmDecryptionSignatureType {
    ...
    for (let i = 0; i < obj.contractAddresses.length; ++i) {
      if (typeof obj.contractAddresses[i] !== "string") return false;
      if (!(obj.contractAddresses[i] as string).startsWith("0x")) return false;
    }
    if (
      !(
        "userAddress" in obj &&
        typeof obj.userAddress === "string" &&
        (obj.userAddress as string).startsWith("0x")
      )
    ) {
      return false;
    }
    if (
      !("eip712" in obj && typeof obj.eip712 === "object" && obj.eip712 !== null)
    ) {
      return false;
    }

    const eip712 = obj.eip712 as Record<string, unknown>;
    if (!("domain" in eip712 && typeof eip712.domain === "object")) {
      return false;
    }
    if (
      !("primaryType" in eip712 && typeof eip712.primaryType === "string")
    ) {
      return false;
    }
    if (!("message" in eip712)) {
      return false;
    }
    if (
      !(
        "types" in eip712 &&
        typeof eip712.types === "object" &&
        eip712.types !== null
      )
    ) {
      return false;
  ...
  static async new(
    ...
    try {
      ...
      console.error("FhevmDecryptionSignature.new() failed!", e);
      return null;
    }
  }
